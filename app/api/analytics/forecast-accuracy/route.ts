...
export async function GET(): Promise<NextResponse<ForecastAccuracyResponse | { error: string }>> {
  ...
  const legacyRecords: ForecastRecord[] = trackedRows.map((r) => {
    const actual = r.actual_load_mw;
  ...
  const mape =
    legacyRecords.reduce((sum, r) => sum + r.errorPct, 0) / legacyRecords.length;
  const rmse = Math.sqrt(
    legacyRecords.reduce(
